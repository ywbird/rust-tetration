...
fn test() {
    let abs = | i: num::complex::Complex<f64> | -> f64 { (i.norm_sqr() as f64).sqrt() };

    let mut real_part = String::new();
    let mut imagin_part = String::new();

    print!("Real part: ");
    io::stdout().flush().unwrap();
    io::stdin()
        .read_line(&mut real_part)
        .expect("Failed to read line");
    
    let real_part: f64 = real_part.trim().parse()
        .expect("Please type a number");

    print!("Real part: ");
    io::stdout().flush().unwrap();
    io::stdin()
        .read_line(&mut imagin_part)
        .expect("Failed to read line");
    
    let imagin_part: f64 = imagin_part.trim().parse()
        .expect("Please type a number");

    let complex_number = num::complex::Complex::new(real_part, imagin_part);

    println!("{}", complex_number);
    println!("scale: {}", abs(complex_number));
